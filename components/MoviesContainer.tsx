...
const MoviesContainer = ({movies,title}:MoviesContainerProps) => {
    const handleMovieClick = async(movie:Movie) => {
        const data  :any = await fetch(`https://api.themoviedb.org/3/${movie.media_type || 'movie'}/${movie.id}/videos?api_key=ec7c8684d689fe7183073ae626036142`).then(res=>res.json())
        ...
        <div className="text-white space-y-4">
           ...
           {movies.map(movie => (
                <div key={movie.id} className=" flex-shrink-0 relative group w-36 h-48" onClick={()=>handleMovieClick(movie)}>
                    <Image src={process.env.NEXT_PUBLIC_TMDB_IMG_BASEURL+'/w500'+movie.poster_path} layout="fill" className="transition duration-300 ease-out group-hover:scale-125"/>
                </div>
            ))}
           </div>
           <div className={`w-full h-[500px] ${!preview && 'hidden'}`}>
